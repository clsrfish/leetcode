...
namespace {
class LeetCodeSuite_107 : public testing::TestWithParam<std::tuple<std::vector<int>, std::vector<std::vector<int>>>> {
 protected:
  leetcode_107::Solution solution;
  LeetCodeSuite_107() {
  }
  ~LeetCodeSuite_107() override {
  }
  void SetUp() override {
    std::vector<int> tmp;
...
TEST_P(LeetCodeSuite_107, Test) {
  auto actual = solution.levelOrderBottom(root);
  ASSERT_EQ(actual, expected);
}

// prefix, test_suite_name
INSTANTIATE_TEST_SUITE_P(leetcode, LeetCodeSuite_107,
